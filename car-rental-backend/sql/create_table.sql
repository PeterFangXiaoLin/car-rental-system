...
(
    ...
    `vehicleDailyPrice`          decimal(10, 2) NOT NULL COMMENT '日租金',
    `totalDays`           int            NOT NULL COMMENT '租赁天数',
    `vehicleTotalAmount`         decimal(10, 2) NOT NULL COMMENT '总金额',
    `needDriver`          tinyint        NOT NULL DEFAULT '0' COMMENT '是否需要司机：0-不需要，1-需要',
    `driverId`            bigint                  DEFAULT NULL COMMENT '司机ID',
    `driverDailyPrice`         decimal(10, 2)          DEFAULT '0.00' COMMENT '司机服务费用/天',
    ...
    `pickupStoreId`      bigint            NOT NULL COMMENT '取车地点',
    `returnStoreId`      bigint            NOT NULL COMMENT '还车地点',
...
(
    ...
    INDEX `uk_userAccount` (`userAccount`),
    UNIQUE KEY `uk_phoneNumber` (`phoneNumber`),
