...
import { addVehicleTypeDictUsingPost } from '@/api/vehicleTypeDictController.ts'

const emit = defineEmits(['success']) // 定义成功事件, 供父组件调用

const visible = ref(false) //
const formRef = ref<FormInstance>()
const formData = ref<API.VehicleTypeDictAddRequest>({})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (valid) {
      ...
      try {
        // 新增
        const res = await addVehicleTypeDictUsingPost(formData.value)
