...
import { createStoreUsingPost } from '@/api/storeController.ts'
...
const formData = ref<API.StoreCreateRequest>({
  images: '',
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (valid) {
      ...
      try {
        // 新增
        const res = await createStoreUsingPost(formData.value)
