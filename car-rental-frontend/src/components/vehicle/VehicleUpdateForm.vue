...
import { getVehicleByIdUsingGet, updateVehicleUsingPost } from '@/api/vehicleController.ts'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (valid) {
      ...
      try {
        // 新增
        const res = await updateVehicleUsingPost(formData.value)
    ...
    ElMessage.error('表单验证失败' + validationError)
  }
}
