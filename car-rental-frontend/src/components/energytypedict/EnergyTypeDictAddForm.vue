...
import { addEnergyTypeDictUsingPost } from '@/api/energyTypeDictController.ts'

const emit = defineEmits(['success']) // 定义成功事件, 供父组件调用

const visible = ref(false) //
const formRef = ref<FormInstance>()
const formData = ref<API.EnergyTypeDictAddRequest>({})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (valid) {
      ...
      try {
        // 新增
        const res = await addEnergyTypeDictUsingPost(formData.value)
