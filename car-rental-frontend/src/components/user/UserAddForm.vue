  ...
  <el-dialog v-model="visible" width="40%" @close="handleClose" destroy-on-close center>
    ...
    >
      <el-form-item label="账号" prop="userAccount">
        <el-input v-model="formData.userAccount" placeholder="请输入用户账号" clearable />
      </el-form-item>
...
import { addUserUsingPost, updateAvatarUsingPost } from '@/api/userController'
...
const formData = ref<API.UserAddRequest>({})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (valid) {
      ...
      try {
        // 新增
        const res = await addUserUsingPost(formData.value)
